...
Sistemas de archivos; archivos, tipos y atributos \\
...
\fancypagestyle{plain}{\fancyhf{}}
  	\pagestyle{fancy}
 	\lhead{\footnotesize Sistemas de archivos; archivos, tipos y atributos - Departamento de Ingeniería de Computadoras}
...
\texttt{
\$ ls -i /tmp\\
658627 ComoAdquirirTarjetaSomosUnco.pdf\\
658640 D is for Digital (2012) Kernighan.epub\\
658629 hellwig-reprint.pdf\\
658620 Horarios\_Tecnicaturas\_2C-2014\_4\_.pdf\\
656656 kde-lechnerm\\
658617 ksocket-lechnerm\\
658633 Linux in the Workplace How to Use Linux in Your Workplace.pdf\\
658615 orbit-lechnerm\\
}
\vspace*{0.4cm} } }
...
\$ ls -1i *junio.pdf \\
5505240 junio.pdf \\
5505240 sueldojunio.pdf \\
\$ find -inum 5505240 \\
./sueldojunio.pdf\\
./junio.pdf\\
\$ find -samefile sueldojunio.pdf \\
./sueldojunio.pdf\\
./junio.pdf\\
\$\\
}
...
Los inodos son almacenados dentro del mismo sistema de archivos, lo cual 
implica que un porcentaje del espacio disponible será utilizado por el 
sistema de archivos en sí mismo. 
...
No son otra cosa que una ``bolsa'' de bytes: los sistemas GNU/Linux no 
impone ninguna estructura a su contenido. Los archivos de texto, de 
datos, programas ejecutables, bibliotecas compartidas, serán todos 
archivos regulares.
...
Los directorios \textit{son archivos} que contienen referencias a otros 
archivos. Podemos crear directorios a través del comando \texttt{mkdir}, 
y eliminarlos a través del comando \texttt{rmdir} si está vacío, o 
\texttt{rm -r} si tiene contenido (o utilizando herramientas gráficas). 

La estructura de un archivo de tipo directorio dependerá del sistema de 
archivos que usemos. En general, podemos pensar un directorio como un 
conjunto de entradas donde se corresponden nombres simbólicos e inodos. 
En particular, las entradas ``.'' y ``..'' de dicho índice harán 
referencia al directorio actual y a su padre respectivamente.  
...
Los \textit{nombres de archivos} contenidos dentro de un directorio, se 
encuentran almacenados en el directorio y no en el archivo en sí mismo. 
Esto permite, entre otras cosas, que un mismo archivo tenga varios nombres 
distintos \textit{dentro del mismo sistema de archivos}. Cada nombre es 
sólo una referencia, el contenido del archivo será el mismo cualquiera sea 
el nombre que usemos para accederlo. Esto es conocido como ``enlace duro''
(en inglés ``hard link''). No debe confundirse un \textit{enlace duro}  
con una copia de archivo por ejemplo creada a través del comando 
\texttt{cp}.  

El primer enlace duro se crea al crear el archivo por medio de alguna 
aplicación. En ese momento, se crea la primer entrada que hace referencia
al archivo, dentro de un archivo de tipo directorio. A partir de ahí, los 
\textit{enlaces duros adicionales} se crearán a través del comando 
\texttt{ln} y se elimina igual que cualquier otro archivo regular, a 
través de el comando \texttt{rm} (o su equivalente gráfico). 
...
El sistema de archivos mantendrá la cuenta de los enlaces duros que existen
a un mismo archivo, decrementando el valor cada vez que se elimina una de 
las referencias, por ejemplo a través del comando \texttt{rm}. Sólo se 
liberará el espacio utilizado por el archivo cuando éste sea la última 
referencia. Podemos ver el número de referencias a un archivo con el 
comando \texttt{stat} o el segundo campo en la salida del comando 
\texttt{ls -l}, entre otros.  
...
Dispositivo: fe01h/65025d	{\bf Nodo-i: 5638106}    \textbf{Enlaces: 1} \\
...
\$ stat martes.txt  \\
  Fichero: «martes.txt» \\
  Tamaño: 27        	Bloques: 8          Bloque E/S: 4096   fichero regular \\
Dispositivo: fe01h/65025d	{\bf Nodo-i: 5638106}    \textbf{Enlaces: 2} \\
Acceso: (0644/-rw-r--r--)  Uid: ( 1000/lechnerm)   Gid: ( 1000/lechnerm) \\
...
Adicionalmente al número de enlaces, observe que el número de inodo 
coincide para todos los enlaces duros. 
\vspace*{0.4cm} } }

Es importante entender que los enlaces duros no son un tipo de archivos 
especial. Simplemente el sistema de archivos permite crear múltiples 
referencias al mismo conjunto de bytes. Tanto el contenido como todos las
propiedades del archivo (propiedad, permisos, tiempos de acceso, etc). 
son compartidos entre todos los enlaces duros.  

\subsection*{Enlace simbólicos}
Un \textit{enlace simbólico o dinámico} (en inglés ``soft link'') será un 
tipo especial de archivo, cuyo contenido hace referencia a un 
\textit{nombre de archivo} dentro de algún sistema de archivos. 
Es decir el contenido de un archivo de tipo link simbólico, es una ruta 
(en inglés ``path'') a otro archivo. La diferencia entre un enlace duro y 
un enlace simbólico radica en que, el enlace duro es un referencia 
directa a un archivo (a la bolsa de bytes), mientras que, un enlace 
simbólico  es una referencia por nombre. \textit{El enlace simbólico
es un archivo diferente del archivo al que apunta}.  


Los enlaces simbólicos se crean a través del comando \texttt{ln -s} 
(se agrega la opción -s -de soft- al comando utilizado para crear enlaces
duros), y se eliminan con el comando \texttt{rm}. La creación o 
eliminación de un link simbólico no altera la cuenta de enlaces del 
...
Dispositivo: fe01h/65025d	{\bf Nodo-i: 5638480}     \textbf{Enlaces: 1} \\
Acceso: (0644/-rw-r--r--)  Uid: ( 1000/lechnerm)   Gid: ( 1000/lechnerm) \\
...
Dispositivo: fe01h/65025d	{\bf Nodo-i: 5638483}     \textbf{Enlaces: 1} \\
...
\$ rm viernes.txt  \\
\$  
Adicionalmente a la cuenta de enlaces, observe que el número de inodo 
correspondiente al enlace simbólico, difiere del número de inodo del 
archivo al cual apunta. 
}
\vspace*{0.4cm} } }

Al almacenar nombres de archivos los enlaces simbólicos, a diferencia de 
los enlaces duros, estos \textit{pueden hacer referencia a archivos en un
sistema de archivos diferente}. A su vez, estas rutas de archivos pueden 
ser absolutas o relativas\footnote{Una ruta absoluta es aquella que 
comienza a partir del directorio raíz /, ej. /usr/share/. Una ruta 
relativa no contiene el directorio raíz / y es relativa al directorio de 
trabajo actual, ej. share/}. 

Es un error común pensar que el primer argumento del comando \texttt{ln -s}
tiene algo que ver con el directorio de trabajo actual. Este no es el 
caso, \texttt{ln} simplemente tomará ese argumento literalmente como el 
destino del enlace simbólico. 

Se debe tener en cuenta que al eliminar el archivo referenciado, el enlace
simbólico sigue existiendo, haciendo referencia a un archivo inexistente. 
